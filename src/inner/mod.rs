...
mod c {
    ...
    pub const AF_UNIX: sa_family_t = ws2def::AF_UNIX as sa_family_t;
...
impl SocketAddr {
    pub fn new<F>(f: F) -> io::Result<SocketAddr>
        where F: FnOnce(*mut c::sockaddr, *mut c_int) -> c_int
    {
        unsafe {
            let mut addr: c::sockaddr_un = mem::zeroed();
    ...
    pub fn from_parts(addr: c::sockaddr_un, mut len: c_int) -> io::Result<SocketAddr> {
        if len == 0 {
